...
class App extends React.Component {
  ...
  handleSubmit(event) {
    ...
    axios.post('http://0.0.0.0:8007/users/', {'email': email, 'username': username, 'password': password}).then(console.log("Success"));
  }
  ...
  handleSubmitLogIn(event) {
    ...
    const password = this.state.password;
    event.preventDefault();
    axios.get('http://0.0.0.0:8007/users/').then(responseArr => {
  ...
  getClasses(){
    axios.get('http://0.0.0.0:8007/classes/').then(responseArr => {
  ...
  handleSubmitCreateClass(event){
    ...
    axios.post('http://0.0.0.0:8007/classes/', {"courseId": courseId, "courseName": courseName, "meetingSchedule": courseMeetingSchedule});
